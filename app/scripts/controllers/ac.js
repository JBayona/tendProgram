...
angular.module('tendProgramApp')
  .controller('AcCtrl', ['$scope', 'Ac', '$q', '$filter', function ($scope, Ac, $q,$filter) {
 		...
 		$scope.deleteFilter = function(index){ //Es recomendable eliminar sobre la propiedad directamente
 			...
 			//Restart the count for the filter date and part
 			for(var i=0; i < $scope.filter.length; i++){
 				if($scope.filter[i].name == 'Date' || $scope.filter[i].name == 'fecha' || $scope.filter[i].name == 'Part' || $scope.filter[i].name == 'parte'){
 					isFilterDate =0;
 				}
 			}
 		...
 		var getTolvaOne = function(){
 			...
 			Ac.getTolva_OneDate().then(function(response){
 				...
 				$scope.searchPage.totalList = [];
 				if($scope.datePart.tolva1.length > 0){
 					$scope.isError = false;
 		...
 		var getTolvaTwo = function(){
 			...
 			Ac.getTolva_TwoDate().then(function(response){
 				...
 				if($scope.datePart.tolva2.length > 0){
 					$scope.isError = false;
 					$scope.searchPage.totalList = [];
 		...
 		var getTolvaThree = function(){
 			...
 			Ac.getTolva_ThreeDate().then(function(response){
 				...
 				if($scope.datePart.tolva3.length > 0){
 					$scope.isError = false;
 					$scope.searchPage.totalList = [];
 		...
 		var getTolvaFour = function(){
 			...
 			Ac.getTolva_FourDate().then(function(response){
 				...
 				if($scope.datePart.tolva4.length > 0){
 					$scope.isError = false;
 					$scope.searchPage.totalList = [];
 		...
 		var getTolvaFive = function(){
 			...
 			Ac.getTolva_FiveDate().then(function(response){
 				...
 				if($scope.datePart.tolva5.length > 0){
 					$scope.isError = false;
 					$scope.searchPage.totalList = [];
 		...
 		var getTolvaSix = function(){
 			...
 			Ac.getTolva_SixDate().then(function(response){
 				...
 				if($scope.datePart.tolva6.length > 0){
 					$scope.isError = false;
 					$scope.searchPage.totalList = [];
 		...
 		var getComp = function(){
 			...
 			Ac.get_COMPDate().then(function(response){
 				...
 				for(var i=0; i < $scope.partType.comp.length; i++){
 					...
 					console.log("Diff=" + differenceDate);
 					if(differenceDate > valueDate){
 						$scope.datePart.comp[indexDate] = $scope.partType.comp[i];
 						indexDate++;
 					}
 				}
 				if($scope.datePart.comp.length > 0){
 					$scope.isError = false;
 					$scope.searchPage.totalList = [];
